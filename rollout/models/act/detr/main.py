...
def get_args_parser():
    ...
    parser.add_argument(
        "--vitacdreamer_cross_attn_layers",
        default="middle",
        type=str,
        help="ACT encoder layers that use policy-token Q cross-attention over ViTac tokens as K,V.",
    )
...
def build_ACT_model_and_optimizer(args_override, RoboTwin_Config=None):
    ...
    vitacdreamer_param_prefixes = (
        "vitacdreamer_feature_proj",
        "vitacdreamer_feature_gate",
    )
    vitacdreamer_adapter_lr = getattr(args, "vitacdreamer_adapter_lr", None)

    param_dicts = []
    main_params = [
        p for n, p in model.named_parameters()
        if "backbone" not in n
        and not n.startswith(vitacdreamer_param_prefixes)
        and p.requires_grad
    ]
    if main_params:
    ...
    vision_backbone_params = [
        p for n, p in model.named_parameters() if n.startswith("backbones.0") and p.requires_grad
    ]
    if vision_backbone_params:
        param_dicts.append({"params": vision_backbone_params, "lr": args.lr_vision_backbone})

    tactile_backbone_params = [
        p for n, p in model.named_parameters() if n.startswith("backbones.1") and p.requires_grad
    ]
    if tactile_backbone_params:
        param_dicts.append({"params": tactile_backbone_params, "lr": args.lr_tactile_backbone})
    if getattr(args, "use_vitacdreamer_feature", False):
        vitacdreamer_params = [
            p for n, p in model.named_parameters()
            if n.startswith(vitacdreamer_param_prefixes) and p.requires_grad
        ]
        if vitacdreamer_params:
            param_dicts.append(
                {
                    "params": vitacdreamer_params,
                    "lr": args.lr if vitacdreamer_adapter_lr is None else vitacdreamer_adapter_lr,
                }
            )
