...
class BlendedActBuffer(ActBuffer):

    def __init__(self, num_act:int, act_dict:Dict[str, tuple], create:bool=False) -> None:
        super().__init__(num_act=num_act, act_dict=act_dict, create=create)

    def get_remaining(self) -> List[Dict[str, np.ndarray]]:
        with self.shm_lock:
            idx = self.shm_idx[:].item()
            remaining = []
            for i in range(idx, self.num_act):
                act = {}
                for name in self.act_names:
                    act[name] = np.copy(self.shm_arrays[i][name])
                remaining.append(act)
        return remaining

    def replace_chunk(self, act:List[Dict[str, np.ndarray]]) -> None:
        assert len(act) == self.num_act
        with self.shm_lock:
            for i in range(len(act)):
                assert set(act[i].keys()) == self.act_names
    ...
    def addn(self, act:List[Dict[str, np.ndarray]], blend_alpha:float = 0.9) -> None:
        assert len(act) == self.num_act
        assert blend_alpha >= 0.0 and blend_alpha <= 1.0
        remaining = self.get_remaining()
        if len(remaining) == 0:
            self.replace_chunk(act)
            return
        overlap_len = len(remaining)
        blended: List[Dict[str, np.ndarray]] = []
        for i in range(overlap_len):
            step = {}
            for name in self.act_names:
                step[name] = (blend_alpha * act[i][name].astype(np.float64)
                              + (1.0 - blend_alpha) * remaining[i][name].astype(np.float64)).astype(act[i][name].dtype)
            blended.append(step)
        blended.extend(act[overlap_len:])
        self.replace_chunk(blended)
