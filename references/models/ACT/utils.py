...
class TacArenaDataset(torch.utils.data.Dataset):
    ...
    vd_image_trans = transforms.Compose([
        transforms.ToTensor(),
        transforms.Resize((224, 224)),
    ])

    @staticmethod
    def _compose_bilateral_tactile(left: torch.Tensor, right: torch.Tensor) -> torch.Tensor:
    ...
    def __init__(
        ...
        vitacdreamer_feature_cache_dir=None,
        vitacdreamer_task_id=None,
    ):
        super().__init__()
        self.episode_ids = episode_ids
        self.dataset_dir = dataset_dir
        ...
        self.norm_stats = norm_stats
        self.use_vitacdreamer_feature = use_vitacdreamer_feature
        self.vitacdreamer_history_len = vitacdreamer_history_len
        self.vitacdreamer_sample_stride = vitacdreamer_sample_stride
        self.vitacdreamer_feature_cache_dir = vitacdreamer_feature_cache_dir
        self.vitacdreamer_task_id = vitacdreamer_task_id
        self._vitac_feature_cache = {}
        self.create_dataset()

    def _zero_vitac_tactile(self) -> torch.Tensor:
    ...
    def __getitem__(self, index):
        ...
        vitacdreamer_inputs = None
        if self.use_vitacdreamer_feature:
            if self.vitacdreamer_feature_cache_dir:
                cache_path = Path(self.vitacdreamer_feature_cache_dir) / f"episode_{episode_idx}.npy"
                if not cache_path.exists():
                    raise FileNotFoundError(
                        f"Missing cached ViTacDreamer feature file: {cache_path}. "
                        "Run precompute_vitacdreamer_features.py first."
                    )
                if episode_idx not in self._vitac_feature_cache:
                    self._vitac_feature_cache[episode_idx] = np.load(cache_path, mmap_mode="r")
                vitacdreamer_inputs = torch.from_numpy(
                    np.asarray(self._vitac_feature_cache[episode_idx][start_ts])
                ).float()
            else:
                visual_history = []
                tactile_history = []
                action_history = []
                history_indices = [
                    start_ts - self.vitacdreamer_sample_stride * (self.vitacdreamer_history_len - offset)
                    for offset in range(self.vitacdreamer_history_len)
                ]

                for hist_ts in history_indices:
                    if hist_ts < 0:
                        visual_history.append(torch.zeros(3, 224, 224))
                        tactile_history.append(self._zero_vitac_tactile())
                        action_history.append(torch.zeros(7))
                        continue
                    visual_history.append(self.vd_image_trans(root["/observations/images/cam_high"][hist_ts]))
                    left_tactile = self.vd_image_trans(root["/observations/images/tac_left"][hist_ts])
                    if "/observations/images/tac_right" not in root:
                        raise KeyError(
                            f"{hdf5_path} is missing /observations/images/tac_right; "
                            "bilateral ViTacDreamer fallback feature path requires both tactile sides."
                        )
                    right_tactile = self.vd_image_trans(root["/observations/images/tac_right"][hist_ts])
                    tactile_history.append(self._compose_bilateral_tactile(left_tactile, right_tactile))
                    action_history.append(torch.from_numpy(root["/action"][hist_ts][:7]).float())

                vitacdreamer_inputs = {
                    "current_tactile": self._compose_bilateral_tactile(
                        self.vd_image_trans(root["/observations/images/tac_left"][start_ts]),
                        self.vd_image_trans(root["/observations/images/tac_right"][start_ts]),
                    ),
                    "visual_history": torch.stack(visual_history, dim=0),
                    "tactile_history": torch.stack(tactile_history, dim=0),
                    "action_history": torch.stack(action_history, dim=0),
                }
                if self.vitacdreamer_task_id is not None:
                    vitacdreamer_inputs["task_id"] = torch.tensor(self.vitacdreamer_task_id, dtype=torch.long)
        ...
        if self.use_vitacdreamer_feature:
            return all_cam_images, all_tac_images, qpos_data, action_data, is_pad, vitacdreamer_inputs
        return all_cam_images, all_tac_images, qpos_data, action_data, is_pad
...
def load_data(
    ...
    train_dataset = TacArenaDataset(
        train_indices, dataset_dir, camera_names, tactile_names, norm_stats, chunk_size,
        use_vitacdreamer_feature=use_vitacdreamer_feature,
        vitacdreamer_history_len=vitacdreamer_history_len,
        vitacdreamer_sample_stride=vitacdreamer_sample_stride,
        vitacdreamer_feature_cache_dir=vitacdreamer_feature_cache_dir,
        vitacdreamer_task_id=vitacdreamer_task_id,
    )
    val_dataset = TacArenaDataset(
        val_indices, dataset_dir, camera_names, tactile_names, norm_stats, chunk_size,
        use_vitacdreamer_feature=use_vitacdreamer_feature,
        vitacdreamer_history_len=vitacdreamer_history_len,
        vitacdreamer_sample_stride=vitacdreamer_sample_stride,
        vitacdreamer_feature_cache_dir=vitacdreamer_feature_cache_dir,
    ...
    return train_dataloader, val_dataloader, norm_stats, True, train_sampler, val_sampler
